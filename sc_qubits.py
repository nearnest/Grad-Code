...
def subtr_e(energies):
    """Takes a list of energies (such as obtained by diagonalization) and returns them after
    subtracting the ground state energy."""
    return (energies - energies[1])

def subtr_f(energies):
    """Takes a list of energies (such as obtained by diagonalization) and returns them after
    subtracting the ground state energy."""
    return (energies - energies[2])

def subtr_f2(energies):
    """Takes a list of energies (such as obtained by diagonalization) and returns them after
    subtracting the ground state energy."""
    return (energies - energies[3])
...
class QubitBaseClass(object):
    ...
    def get_evals_vs_paramvals(self, param, paramval_list, evnum=6, subtract_ground=False, subtract_excited=False):
        ...
        for ind, paramval in enumerate(paramval_list):
            ...
            evals = np.sort(self.eigenvals(evnum))
            specdata[ind] = evals
            if subtract_ground:
                specdata[ind] = evals - evals[0]
            elif subtract_excited:
                specdata[ind] = evals - evals[1]
            else:
                specdata[ind] = evals
    ...
    def plot_transitions_vs_paramvals(self, param, paramval_list, evnum=6,
                                yrange=False, subtract_ground=False, shift=0, to_file=None):
        """Generates a simple plot of a set of eigenvalues as a function of parameter 'param'.
        The individual points correspond to the parameter values listed in paramval_list.
        ...
        """
        x = paramval_list
        evs = np.transpose(self.get_evals_vs_paramvals(param, paramval_list, evnum, subtract_ground))
#        if not yrange:
#            plt.axis([np.amin(x), np.amax(x), np.amin(y + shift), np.amax(y + shift)])
#        else:
#            plt.axis([np.amin(x), np.amax(x), yrange[0], yrange[1]])
        plt.xlabel(param)
        plt.ylabel("energy")
        for ii,evi in enumerate(evs):
            for jj, evj in enumerate(evs[:ii]):
                if ii>=4 and ii<7:
                    lines = plt.plot(x,evi-evj+shift, '--', label='E%d%d' % (jj,ii))
